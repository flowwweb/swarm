...
Implementation map: `9ee8d85` heartbeat/archive/contention/dependency baseline; `f8afc0a` adaptive direct paths and WARM reuse; `edd6749` configured heartbeat threshold; `0271fb4` initial SWARM-first compatibility; `eccb957` artifact/HIVE/archive authority; `9f97855` total heartbeat recovery incident budget and latch handling; `9fc5585` initial instruction-architecture distillation; `212ca38` canonical artifact provenance enforcement; `b05ea11` executable Scope Discipline finding behavior; `5acfb81` live-owner archive matrix, complete context transfer accounting, typed artifact justification, per-task hop receipts, centralized resolver used by console/Docker, and bounded routine receipts; `a561bf5` compact always-loaded CORE/ROLE/TASK/EVENT and task contract; `4324561` canonical SWARM config template with clearly labeled legacy fallback; `ac6b652` complete SWARM-first copy and compatibility labeling; `8d0b2a9` canonical heartbeat recovery, HIVE package filtering, typed duplicate provenance, conservative depth, shared Docker config resolution, and semantic CTRL events. P3/no-change console redesign and optimizer work remain untouched.
