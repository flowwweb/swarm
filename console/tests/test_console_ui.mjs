...
assert.match(app, /String\(role\?\.id \|\| ""\)\.toLowerCase\(\) === "dev" \? "Developer"/);
...
assert.match(css, /\.role-library-layout \{[^}]*grid-template-columns:minmax\(220px,300px\) minmax\(0,1fr\)/);
assert.match(css, /\.role-choice \{[^}]*min-height:60px/);
assert.match(css, /@media \(max-width: 620px\)[\s\S]*\.role-library-layout \{ grid-template-columns:1fr; \}[\s\S]*\.role-choice \{ min-height:56px; \}/);
