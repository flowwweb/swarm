...
function usageSaverTunnelMarkup(enabled) {
  if (!enabled) return "";
  const setup = state.usageSaverTunnel;
  if (setup?.state === "CONNECTED") {
    return '<div class="usage-saver-tunnel is-ready"><strong>ChatGPT connected</strong><small>SWARM MCP verified' + (setup.verified_at_ms ? ' · ' + escapeHTML(formatRelative(setup.verified_at_ms)) : '') + '</small></div>';
  }
  const steps = Array.isArray(setup?.steps) ? setup.steps : ["Create an OpenAI secure MCP tunnel.", "Add the SWARM plugin in ChatGPT.", "Connect once and run a status check."];
  return '<details class="usage-saver-tunnel"><summary>Connect ChatGPT</summary><ol>' + steps.map((step) => '<li>' + escapeHTML(step) + '</li>').join('') + '</ol><small>The API key stays in your environment, never the SWARM database.</small></details>';
}
...
function renderSettings() {
  ...
  const usageSaver = settingsDraftValue("execution.usage_saver", state.config?.settings?.execution?.usage_saver === true);
...
async function refreshUsageSaverTunnel() {
  try { state.usageSaverTunnel = await api('/api/usage-saver-tunnel'); }
  catch { state.usageSaverTunnel = null; }
}
...
async function refreshOverview(showLoading = true) {
  ...
  try {
    ...
    await Promise.all([refreshProof(), refreshUsageHistory(), refreshUsageSaverTunnel(), refreshProjectProgress(), refreshProjectProgressFeed(), refreshRoleManifests(), refreshLabs(), refreshNotifications(), refreshRunLogs(), refreshAssets(), refreshProfileSummary(), refreshCommandApprovals(), refreshMessageConversation()]);
