...
An HQ execution request is authoritative only when an injected host verifier accepts either a single-use receipt for its immutable `HQCommandEnvelope` or a current scoped AUTO-only grant. Constructed records carry no authority. The envelope binds action, project, canonical root digest, CTRL where applicable, target intent/thread, exact active turn for steer, payload digest, expected Ledger revision, acknowledgement requirement, submission time, and expiry. Injected ephemeral dispatch material supplies the canonical cwd and instruction bytes; the instruction's UTF-8 SHA-256 and a separate host observation binding that exact cwd to the authorized root must both pass before Ledger COMMAND reservation or transport. New, resumed, and repair work acknowledges one unambiguous string thread identity only after the returned thread cwd is independently observed and bound to the authorized root; only an unambiguous matching turn response may produce RESULT. App Server cwd is verifier input, not root authority. An ambiguous lifecycle remains pending, and exact retained COMMAND/ACK reconciliation requires a fresh root-bound host thread observation but not expired submission authorization or transient material; it never redispatches. CONNECTOR receipts retain safe digests and verified host thread/turn/root identities only; they grant no progress, proof, review, or acceptance authority and never retain prompts, responses, credentials, or provider payloads.
