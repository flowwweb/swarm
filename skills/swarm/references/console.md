...
is off by default, applies to new RUSH actions, and does not change model routing,
...
```powershell
python skills/rush/scripts/rush_console.py --open
```
...
Direct Compose use must set `RUSH_CODEX_HOME` and `RUSH_CONFIG_HOME` explicitly.

Compose publishes only `127.0.0.1:4788`, mounts Codex metadata read-only, and
mounts the RUSH config directory for validated settings writes. Do not expose the
container port to a LAN or public host.

## Authority and privacy

- The console derives hierarchy from Codex task spawn edges and short
  RUSH-formatted task titles. It does not create, accept, steer, or archive tasks.
...
- Config writes are same-origin, token-gated, allowlisted, atomically replaced,
  and accepted only after the existing RUSH validator passes. One prior config
