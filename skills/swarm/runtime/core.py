...
def pin_policy(
    ...
    *,
    ...
    User custody wins before any role or lifecycle default.  The host task API
    must independently authorize any returned mutation request.
    """
...
class ControlPathFailureKind(StrEnum): EMPTY_COMPLETION="EMPTY_COMPLETION"; SILENCE="SILENCE"; UNREADABLE="UNREADABLE"; TIMEOUT="TIMEOUT"; MISSING_THREAD="MISSING_THREAD"
class RecoveryCause(StrEnum): CONTROL_TRANSPORT="CONTROL_TRANSPORT"; OWNER_CAPACITY="OWNER_CAPACITY"; DEPENDENCY="DEPENDENCY"; PROOF_REVIEW="PROOF_REVIEW"; ENVIRONMENT_RESOURCE="ENVIRONMENT_RESOURCE"; EXTERNAL_SERVICE="EXTERNAL_SERVICE"; USER_AUTHORITY="USER_AUTHORITY"; SAFETY_DESTRUCTIVE="SAFETY_DESTRUCTIVE"
class ControlPathRecoveryAction(StrEnum): SAME_OWNER_DIFFERENT_ROUTE="SAME_OWNER_DIFFERENT_ROUTE"; AUTHORIZED_OWNER_HANDOFF="AUTHORIZED_OWNER_HANDOFF"; WAIT_FOR_RELEASE="WAIT_FOR_RELEASE"; NEEDS_AUTHORITY="NEEDS_AUTHORITY"; ROOT_CAUSE_REASSESSMENT="ROOT_CAUSE_REASSESSMENT"; USER_KEEP_OUT="USER_KEEP_OUT"; TERMINAL_BLOCKED="TERMINAL_BLOCKED"
class ControlPathState(StrEnum):
    CONTROL_PATH_FAILURE="CONTROL_PATH_FAILURE"; RECOVERING="RECOVERING"; WAITING="WAITING"; NEEDS_AUTHORITY="NEEDS_AUTHORITY"; STALLED="STALLED"; BLOCKED="BLOCKED"
    @classmethod
    def from_historical(cls, value:str) -> "ControlPathState": return cls(value)
class ControlPathTruth(StrEnum): UNVERIFIED="UNVERIFIED"
...
def _route_candidate(*, facts:WorkRoutingFacts, economics:RoutingEconomics, capacity:HostCapacityEvidence, accountable_owner:str, lead_owner:str, immutable_checkpoint:str, resumption_marker:str, affected_gates:tuple[str,...]) -> ExecutionRoutingDecision:
    ...
    normal_subagent=facts.size is WorkSize.SMALL and facts.bounded and facts.low_risk and facts.mutable_surface_count==1 and not visible_task_required and economics.critical_path_savings_ms<=economics.task_overhead_ms
...
class AttemptCostReceipt:
    ...
    failure_identity:str=""; operation:str=""; attempt:int=0
    def __post_init__(self):
        ...
        bindings=(bool(self.failure_identity),bool(self.operation),bool(self.attempt))
        if any(bindings) and not all(bindings): raise InvariantError("cost receipt control-path binding requires failure identity, operation, and attempt")
        if self.failure_identity: object.__setattr__(self,"failure_identity",_require_digest(self.failure_identity,"cost receipt failure identity")); _safe_token(self.operation)
        if not isinstance(self.attempt,int) or isinstance(self.attempt,bool) or self.attempt<0: raise InvariantError("cost receipt attempt must be a nonnegative integer")
...
@dataclass(frozen=True)
class ControlPathFailure:
    task_id:str; owner_id:str; dispatch_receipt_id:str; completion_receipt_id:str; kind:ControlPathFailureKind; worktree:str; artifact:ArtifactIdentity; artifact_paths:tuple[str,...]; failed_route:str; observed_at_ms:int; narration:str=""
    cause:RecoveryCause=RecoveryCause.CONTROL_TRANSPORT; affected_edges:tuple[str,...]=()
    def __post_init__(self):
        _safe_token(self.task_id); _safe_token(self.owner_id)
        _require_receipt_text(self.dispatch_receipt_id,"control-path dispatch receipt"); _require_receipt_text(self.completion_receipt_id,"control-path completion receipt")
        if not isinstance(self.kind,ControlPathFailureKind): raise InvariantError("control-path failure requires a typed transport outcome")
        object.__setattr__(self,"worktree",_bounded_readable(self.worktree,"control-path worktree",1024))
        if not isinstance(self.artifact,ArtifactIdentity): raise InvariantError("control-path failure requires the exact candidate artifact")
        object.__setattr__(self,"artifact_paths",_portable_paths(self.artifact_paths,"control-path artifact paths"))
        _safe_token(self.failed_route)
        if not isinstance(self.observed_at_ms,int) or isinstance(self.observed_at_ms,bool) or self.observed_at_ms<0: raise InvariantError("control-path failure requires a nonnegative observation time")
        if not isinstance(self.narration,str): raise InvariantError("control-path narration must be text")
        if not isinstance(self.cause,RecoveryCause): raise InvariantError("recovery requires a typed cause")
        edges=tuple(self.affected_edges) or (self.task_id,)
        if any(not isinstance(edge,str) or _safe_token(edge)!=edge for edge in edges) or len(set(edges))!=len(edges): raise InvariantError("recovery requires distinct exact affected edges")
        object.__setattr__(self,"affected_edges",edges)
    @property
    def signature_digest(self) -> str:
        payload=(self.task_id,self.owner_id,self.dispatch_receipt_id,self.completion_receipt_id,self.kind.value,self.worktree,self.artifact.key(),self.artifact_paths,self.failed_route,self.cause.value,self.affected_edges)
        return _sha256_text(json.dumps(payload,separators=(",",":"),ensure_ascii=True))

@dataclass(frozen=True)
class ControlPathRecoveryDecision:
    ...
    def __post_init__(self):
        ...
        if self.state is ControlPathState.BLOCKED and (not self.release_condition or not self.responsible_authority or not self.release_receipt_id): raise InvariantError("terminal BLOCKED requires an exact release condition, authority, and receipt")
        if self.release_receipt_id: _require_receipt_text(self.release_receipt_id,"terminal recovery release receipt")
        if self.state is not ControlPathState.BLOCKED and self.release_receipt_id: raise InvariantError("only terminal BLOCKED may retain a release receipt")
        if self.state is ControlPathState.NEEDS_AUTHORITY and not self.responsible_authority: raise InvariantError("NEEDS_AUTHORITY requires the exact responsible authority")
        if len(set(self.permitted_routes))!=len(self.permitted_routes) or any(not isinstance(route,str) or not route.strip() for route in self.permitted_routes): raise InvariantError("recovery permitted routes must be distinct readable identities")
        object.__setattr__(self,"next_check_or_release",_bounded_readable(self.next_check_or_release,"recovery next check or release",1024))
        if self.last_good_checkpoint is not None and not isinstance(self.last_good_checkpoint,MaterialStateReceipt): raise InvariantError("recovery last-good checkpoint must be typed")
        if self.eta_range_ms is not None:
            start,end=self.eta_range_ms
    ...
    @property
    def terminal_blocked(self) -> bool: return self.action is ControlPathRecoveryAction.TERMINAL_BLOCKED

@dataclass(frozen=True)
class _ControlPathCustodyProof:
    task_id:str; generation_digest:str; active:bool; release_receipt_id:str=""
    _authority:object|None=field(default=None,init=False,repr=False,compare=False)
    def __post_init__(self):
        _safe_token(self.task_id); object.__setattr__(self,"generation_digest",_require_digest(self.generation_digest,"control-path custody generation"))
        if not isinstance(self.active,bool): raise InvariantError("control-path custody proof requires boolean derived state")
        if self.release_receipt_id: _require_receipt_text(self.release_receipt_id,"control-path release receipt")
...
class RetryTopologyLedger:
    ...
    def _issue_custody_proof(self, task_id:str, generation_digest:str, active:bool, release_receipt_id:str="") -> _ControlPathCustodyProof:
        proof=_ControlPathCustodyProof(task_id,generation_digest,active,release_receipt_id); object.__setattr__(proof,"_authority",self._host_authority); return proof

    def control_path_release_binding(self, failure:ControlPathFailure, *, same_owner_route:str="", safely_resumable:bool=False, authorized_handoff_owner:str="", release_condition:str, responsible_authority:str) -> str:
        """Bind a host release receipt to the exact retained and prospective exhaustion facts."""
        if not isinstance(failure,ControlPathFailure): raise InvariantError("terminal recovery release requires a typed failure")
        ...
        route=same_owner_route.strip(); handoff=authorized_handoff_owner.strip()
        permitted=tuple(candidate for candidate in (route if safely_resumable and route and route!=failure.failed_route else "",f"handoff-{handoff}" if handoff else "") if candidate)
        failed_routes=set(self._control_path_failed_routes.get(tactic_digest,set())); failed_routes.add(failure.failed_route)
        known_routes=set(self._control_path_permitted_routes.get(tactic_digest,set())); known_routes.update(permitted)
        turn_receipts=set(self._control_path_attempt_receipts.get(tactic_digest,set())); turn_receipts.add(failure.completion_receipt_id)
        return _sha256_text(json.dumps(("control-path-terminal-release",failure.task_id,tactic_digest,tuple(sorted(known_routes)),tuple(sorted(failed_routes)),tuple(sorted(turn_receipts)),_bounded_readable(release_condition,"terminal recovery release condition",1024),_safe_token(responsible_authority)),separators=(",",":"),ensure_ascii=True))
    ...
    @staticmethod
    def _cost_snapshot(receipt:AttemptCostReceipt) -> tuple[object,...]:
        return (receipt.receipt_id,receipt.token_count,receipt.latency_ms,receipt.failure_identity,receipt.operation,receipt.attempt)

    @staticmethod
    def _cost_from_snapshot(payload:object) -> AttemptCostReceipt:
        try: return AttemptCostReceipt(*payload)  # type: ignore[arg-type]
        except (TypeError,ValueError) as error: raise InvariantError("control-path cost snapshot is malformed") from error

    @staticmethod
    def _failure_snapshot(failure:ControlPathFailure) -> dict[str,object]:
        artifact=failure.artifact
        return {"task_id":failure.task_id,"owner_id":failure.owner_id,"dispatch_receipt_id":failure.dispatch_receipt_id,"completion_receipt_id":failure.completion_receipt_id,"kind":failure.kind.value,"worktree":failure.worktree,"artifact":(artifact.base,artifact.revision,artifact.purpose,artifact.observables,artifact.observed_paths),"artifact_paths":failure.artifact_paths,"failed_route":failure.failed_route,"observed_at_ms":failure.observed_at_ms,"narration":failure.narration,"cause":failure.cause.value,"affected_edges":failure.affected_edges}

    @classmethod
    def _failure_from_snapshot(cls, payload:object) -> ControlPathFailure:
        keys={"task_id","owner_id","dispatch_receipt_id","completion_receipt_id","kind","worktree","artifact","artifact_paths","failed_route","observed_at_ms","narration","cause","affected_edges"}
        if not isinstance(payload,dict) or set(payload)!=keys: raise InvariantError("control-path failure snapshot has an invalid schema")
        try:
            base,revision,purpose,observables,observed_paths=payload["artifact"]  # type: ignore[misc]
            artifact=ArtifactIdentity(base,revision,purpose,tuple(tuple(item) for item in observables),tuple(observed_paths))
            return ControlPathFailure(payload["task_id"],payload["owner_id"],payload["dispatch_receipt_id"],payload["completion_receipt_id"],ControlPathFailureKind(payload["kind"]),payload["worktree"],artifact,tuple(payload["artifact_paths"]),payload["failed_route"],payload["observed_at_ms"],payload["narration"],RecoveryCause(payload["cause"]),tuple(payload["affected_edges"]))  # type: ignore[arg-type]
        except (TypeError,ValueError) as error: raise InvariantError("control-path failure snapshot is malformed") from error
    ...
    def from_control_path_snapshot(cls, payload:dict[str,object]) -> "RetryTopologyLedger":
        ...
        try:
            ...
            failed={_require_digest(identity,"control-path failed-route identity"):{_safe_token(route) for route in routes} for identity,routes in payload["failed_routes"]}  # type: ignore[union-attr]
            permitted={_require_digest(identity,"control-path permitted-route identity"):{_safe_token(route) for route in routes} for identity,routes in payload["permitted_routes"]}  # type: ignore[union-attr]
            receipts={_require_digest(identity,"control-path receipt identity"):{_require_receipt_text(receipt,"control-path goal-turn receipt") for receipt in values} for identity,values in payload["attempt_receipts"]}  # type: ignore[union-attr]
    ...
    def _retain_control_cost(self, receipt:AttemptCostReceipt|None, failure:ControlPathFailure, attempt:int) -> tuple[int|None,int|None]:
        tokens,latency=self._validate_control_cost(receipt,failure,attempt)
        if receipt is not None: self.cost_receipts[receipt.receipt_id]=receipt
        return tokens,latency

    def _validate_control_cost(self, receipt:AttemptCostReceipt|None, failure:ControlPathFailure, attempt:int) -> tuple[int|None,int|None]:
        if receipt is None: return None,None
        if (receipt.failure_identity,receipt.operation,receipt.attempt)!=(failure.signature_digest,failure.failed_route,attempt): raise InvariantError("control-path cost receipt must bind the exact failure identity, operation, and attempt")
        retained=self.cost_receipts.get(receipt.receipt_id)
        if retained is not None and retained!=receipt: raise InvariantError("cost receipt identity conflicts with retained observation")
        return receipt.token_count,receipt.latency_ms

    def _prospective_material(self, receipt:MaterialStateReceipt|None) -> tuple[bool,MaterialStateReceipt|None]:
        current=self.last_good_checkpoint
        if receipt is None: return False,current
        if current is None or current.content_address!=receipt.content_address:
    ...
    def _retain_material(self, receipt:MaterialStateReceipt|None) -> bool:
        ...
        if current is None or current.content_address!=receipt.content_address:
            if current is not None and receipt.observed_at_ms<=current.observed_at_ms: raise InvariantError("material state receipt must advance the observed high-water")
            self.last_good_checkpoint=receipt; self._attempts.clear(); self._signatures.clear(); self._pending=None; self._reassessment_used=False; self._control_path_attempts.clear(); self._control_path_failed_routes.clear(); self._control_path_permitted_routes.clear(); self._control_path_attempt_receipts.clear(); self._control_path_last_observed_ms.clear(); return True
        if receipt.observed_at_ms>current.observed_at_ms: self.last_good_checkpoint=receipt
        return False
    ...
    def observe(self, *, action:str, target:str, outcome:RetryOutcome, blocker_code:str="", narration:str="", material_receipt:MaterialStateReceipt|None=None, cost_receipt:AttemptCostReceipt|None=None, user_paused:bool=False, user_controlled:bool=False) -> RetryTopologyDecision:
        ...
        progressed=self._retain_material(material_receipt)
    ...
    def control_path_failure(self, failure:ControlPathFailure, *, same_owner_route:str="", safely_resumable:bool=False, authorized_handoff_owner:str="", successor_prohibited:bool=False, replacement_prohibited:bool=False, disjoint_ready_task_ids:tuple[str,...]=(), custody_proof:object|None=None, release_condition:str="", responsible_authority:str="", release_receipt:HostCustodyReceipt|None=None, release_receipt_id:str="", eta_range_ms:tuple[int,int]|None=None, eta_receipt_id:str="", material_receipt:MaterialStateReceipt|None=None, cost_receipt:AttemptCostReceipt|None=None) -> ControlPathRecoveryDecision:
        """Classify one receipt-less completion and choose one bounded recovery without polling."""
        if not isinstance(failure,ControlPathFailure): raise InvariantError("control-path recovery requires a typed failure")
        if not isinstance(custody_proof,_ControlPathCustodyProof) or custody_proof._authority is not self._host_authority or custody_proof.task_id!=failure.task_id: raise InvariantError("control-path recovery requires an internal typed task-custody proof")
        if release_receipt is not None:
            expected_release=self.control_path_release_binding(failure,same_owner_route=same_owner_route,safely_resumable=safely_resumable,authorized_handoff_owner=authorized_handoff_owner,release_condition=release_condition,responsible_authority=responsible_authority)
            if not isinstance(release_receipt,HostCustodyReceipt) or custody_proof.release_receipt_id!=release_receipt.receipt or release_receipt.mutation is not CustodyMutation.STATE or release_receipt.target_id!=failure.task_id or release_receipt.target_state_digest!=expected_release or release_receipt.issued_at<failure.observed_at_ms: raise InvariantError("terminal recovery requires a retained host-verified release receipt bound to exhausted routes and goal turns")
        ...
        if release_receipt_id: raise InvariantError("terminal recovery rejects caller-authored release receipt strings")
        ...
        if prior is not None:
            ...
            return replay
        route=same_owner_route.strip()
        handoff=authorized_handoff_owner.strip()
        permitted=tuple(candidate for candidate in (route if safely_resumable and route and route!=failure.failed_route else "",f"handoff-{handoff}" if handoff else "") if candidate)
        failed_routes=set() if material_reentry else set(self._control_path_failed_routes.get(tactic_digest,set())); failed_routes.add(failure.failed_route)
        known_routes=set() if material_reentry else set(self._control_path_permitted_routes.get(tactic_digest,set())); known_routes.update(permitted)
        ...
        if custody_proof.active:
            ...
            _safe_token(route)
            if route==failure.failed_route: raise InvariantError("same-owner recovery must use a materially different receipt route")
            action=ControlPathRecoveryAction.SAME_OWNER_DIFFERENT_ROUTE; handoff=""; route_rank=0; signal=WatchdogSignal.ATTENTION; state=ControlPathState.RECOVERING; next_check=release_condition.strip() or f"verify:{route}"
        elif handoff:
            _safe_token(handoff); action=ControlPathRecoveryAction.AUTHORIZED_OWNER_HANDOFF; route=""; route_rank=1; signal=WatchdogSignal.ATTENTION; state=ControlPathState.RECOVERING; next_check=release_condition.strip() or f"receipt:{handoff}"
        ...
        elif failures>=3 and known_routes and len(failed_routes)>=3 and not unexhausted_routes and release_condition.strip() and responsible_authority.strip() and release_receipt is not None:
            release=_bounded_readable(release_condition,"terminal recovery release condition",1024); authority=_safe_token(responsible_authority); release_receipt_id=_require_receipt_text(release_receipt.receipt,"terminal recovery release receipt")
            ...
            action=ControlPathRecoveryAction.ROOT_CAUSE_REASSESSMENT; route="root-cause-reassessment"; handoff=""; route_rank=2; signal=WatchdogSignal.ATTENTION; state=ControlPathState.STALLED; next_check=release_condition.strip() or "verify:materially-different-route"
        else:
            if not successor_prohibited or not replacement_prohibited: raise InvariantError("control-path recovery requires an alternate route, an authorized handoff, or exact successor/replacement keep-out")
            authority=_safe_token(responsible_authority); action=ControlPathRecoveryAction.NEEDS_AUTHORITY; route=""; handoff=""; route_rank=4; signal=WatchdogSignal.ATTENTION; state=ControlPathState.NEEDS_AUTHORITY; next_check=release_condition.strip() or f"authority:{authority}"
        if action not in {ControlPathRecoveryAction.TERMINAL_BLOCKED,ControlPathRecoveryAction.WAIT_FOR_RELEASE}: release=""
        if action not in {ControlPathRecoveryAction.TERMINAL_BLOCKED,ControlPathRecoveryAction.NEEDS_AUTHORITY}: authority=""
        if action is not ControlPathRecoveryAction.TERMINAL_BLOCKED: release_receipt_id=""
...
class TaskStartReceipt:
    host_receipt:HostCustodyReceipt; goal_id:str; task_id:str; owner_id:str; started_at_ms:int; scope_version:int=1; lease_version:int=1
    def __post_init__(self):
        if not isinstance(self.host_receipt,HostCustodyReceipt): raise InvariantError("task start requires a typed host custody receipt")
        for value in (self.goal_id,self.task_id,self.owner_id): _safe_token(value)
...
class Swarm:
    ...
    def _task_handoff_ledger(self):
        ...
        if ledger is None or not all(hasattr(ledger,name) for name in ("append_task_handoff","project_task_handoffs","retain_host_custody_receipt","require_host_custody_receipt")): raise InvariantError("task continuity requires the existing Ledger authority and host custody verifier")
        return ledger
    @staticmethod
    def _handoff_id(start:TaskStartReceipt) -> str:
        digest=sha256(json.dumps((start.goal_id,start.task_id,start.owner_id,start.scope_version,start.lease_version,start.host_receipt.receipt),separators=(",",":")).encode()).hexdigest(); return f"handoff-{digest}"
    def _handoff_event(self, current:dict|None, kind:str, *, handoff_id:str, goal_id:str, task_id:str, old_owner:str, new_owner:str|None, checkpoint_digest:str|None, scope_version:int, lease_version:int, receipt_id:str, observed_at_ms:int, host_issued_at_ms:int|None=None) -> dict:
        event_id=f"handoff-event-{sha256(f'{handoff_id}:{kind}'.encode()).hexdigest()}"
        return {"schema_version":1,"record_type":"TASK_HANDOFF","event_id":event_id,"dedupe_key":f"handoff-dedupe-{sha256(f'{handoff_id}:{kind}'.encode()).hexdigest()}","handoff_id":handoff_id,"parent_event_id":None if current is None else current["event_id"],"event_kind":kind,"goal_id":goal_id,"task_id":task_id,"old_owner":old_owner,"new_owner":new_owner,"checkpoint_digest":checkpoint_digest,"scope_version":scope_version,"lease_version":lease_version,"receipt_id":receipt_id,"host_issued_at_ms":host_issued_at_ms,"observed_at_ms":observed_at_ms}
    def _require_handoff_host_receipt(self, receipt:HostCustodyReceipt, payload:dict) -> None:
        from .progress_events import task_handoff_host_binding
        self._task_handoff_ledger().require_host_custody_receipt(receipt,target_id=payload["task_id"],binding=task_handoff_host_binding(payload),issued_at=payload["host_issued_at_ms"])
    def task_handoff_due(self, actor:Role, task_id:str, start:TaskStartReceipt, *, now_ms:int) -> dict:
        ...
        if (start.goal_id,start.task_id,start.owner_id,start.scope_version,start.lease_version)!=(task.goal_id,task.id,task.owner,task.objective_version,task.current_lease_version): raise InvariantError("task lifetime requires a bound current task start receipt")
        ...
        handoff_id=self._handoff_id(start); start_event=self._handoff_event(None,"HANDOFF_DUE",handoff_id=handoff_id,goal_id=task.goal_id,task_id=task.id,old_owner=task.owner,new_owner=None,checkpoint_digest=None,scope_version=task.objective_version,lease_version=start.lease_version,receipt_id=start.host_receipt.receipt,host_issued_at_ms=start.started_at_ms,observed_at_ms=deadline); self._require_handoff_host_receipt(start.host_receipt,start_event)
        if now_ms<deadline: return {"state":"RUNNING","due_at_ms":deadline,"handoff_id":None}
        ledger=self._task_handoff_ledger(); current=next((item for item in ledger.project_task_handoffs()["records"] if item["handoff_id"]==handoff_id),None)
        if current is None:
            ledger.append_task_handoff(start_event,custody_receipt=start.host_receipt); current=next(item for item in ledger.project_task_handoffs()["records"] if item["handoff_id"]==handoff_id)
    ...
    def offer_task_handoff(self, actor:Role, handoff_id:str, *, new_owner:str, checkpoint_digest:str, observed_at_ms:int) -> dict:
        ...
        if task is None or task.owner!=current["old_owner"] or task.objective_version!=current["scope_version"] or task.current_lease_version!=current["lease_version"]: raise InvariantError("handoff offer conflicts with current task custody")
    ...
    def acknowledge_task_handoff(self, actor:Role, handoff_id:str, *, new_owner:str, checkpoint_digest:str, host_task_receipt:HostCustodyReceipt, observed_at_ms:int) -> dict:
        self._role(actor,{Role.DOER,Role.LEAD})
        if not isinstance(host_task_receipt,HostCustodyReceipt): raise InvariantError("handoff acknowledgement requires a typed host-minted custody receipt")
        ledger=self._task_handoff_ledger(); current=next((item for item in ledger.project_task_handoffs()["records"] if item["handoff_id"]==handoff_id),None)
        ...
        if current["event_kind"]=="HANDOFF_ACKNOWLEDGED":
            expected=(current["new_owner"],current["checkpoint_digest"],current["receipt_id"],current["host_issued_at_ms"],current["observed_at_ms"])
            actual=(new_owner,checkpoint_digest,getattr(host_task_receipt,"receipt",None),getattr(host_task_receipt,"issued_at",None),observed_at_ms)
            if expected!=actual: raise InvariantError("handoff acknowledgement replay conflicts with retained identity")
            self._require_handoff_host_receipt(host_task_receipt,current)
        ...
        task=self.tasks.get(current["task_id"]); worker=self.workers.get(new_owner)
        if task is None or task.owner!=current["old_owner"] or task.current_lease_version!=current["lease_version"] or worker is None or worker.state is WorkerState.RETIRED or current["new_owner"]!=new_owner or current["checkpoint_digest"]!=checkpoint_digest: raise InvariantError("handoff acknowledgement conflicts with task, owner, lease, or checkpoint")
        payload=self._handoff_event(current,"HANDOFF_ACKNOWLEDGED",handoff_id=handoff_id,goal_id=current["goal_id"],task_id=current["task_id"],old_owner=current["old_owner"],new_owner=new_owner,checkpoint_digest=checkpoint_digest,scope_version=current["scope_version"],lease_version=current["lease_version"],receipt_id=host_task_receipt.receipt,host_issued_at_ms=host_task_receipt.issued_at,observed_at_ms=observed_at_ms); self._require_handoff_host_receipt(host_task_receipt,payload); return ledger.append_task_handoff(payload,custody_receipt=host_task_receipt)
    def transfer_task_custody(self, actor:Role, handoff_id:str, *, observed_at_ms:int) -> dict:
        ...
        task=self.tasks.get(current["task_id"]); old=self.workers.get(current["old_owner"]); new=self.workers.get(current["new_owner"])
        if task is None: raise InvariantError("custody transfer task is unavailable")
        if new is None or new.id!=current["new_owner"] or new.state is WorkerState.RETIRED: raise InvariantError("custody transfer target is unavailable or mismatched")
        if old is not None and old.id!=current["old_owner"]: raise InvariantError("custody transfer source is mismatched")
        expected_lease=current["lease_version"] if task.owner==current["old_owner"] else current["lease_version"]+1
        if task.goal_id!=current["goal_id"] or task.objective_version!=current["scope_version"] or task.current_lease_version!=expected_lease or task.owner not in {current["old_owner"],current["new_owner"]}: raise InvariantError("custody transfer conflicts with prospective task state")
        if current["event_kind"]=="HANDOFF_ACKNOWLEDGED":
        ...
        if task.owner==current["old_owner"]:
            ...
            new.task_ids.add(task.id); task.owner=current["new_owner"]; task.current_lease_version=current["lease_version"]+1
    ...
    def reconcile_task_handoffs(self) -> tuple[str,...]:
        ...
        for current in ledger.project_task_handoffs()["records"]:
            ...
            old=self.workers.get(current["old_owner"]); new=self.workers.get(current["new_owner"]) if current["new_owner"] else None
            bindings_match=task.goal_id==current["goal_id"] and task.objective_version==current["scope_version"] and isinstance(current["lease_version"],int) and current["lease_version"]>=1 and old is not None and old.id==current["old_owner"]
            if current["event_kind"]=="CUSTODY_TRANSFERRED":
                target_valid=new is not None and new.id==current["new_owner"] and new.state is not WorkerState.RETIRED
                old_state=task.owner==current["old_owner"] and task.current_lease_version==current["lease_version"] and task.id in old.task_ids and (new is None or task.id not in new.task_ids)
                new_state=target_valid and task.owner==current["new_owner"] and task.current_lease_version==current["lease_version"]+1 and task.id in new.task_ids and task.id not in old.task_ids
                if not bindings_match or not target_valid or not (old_state or new_state): continue
                if old_state:
                    old.task_ids.discard(task.id); new.task_ids.add(task.id); task.owner=current["new_owner"]; task.current_lease_version=current["lease_version"]+1; reconciled.append(task.id)
                task.handoff_active=False
            elif bindings_match and task.owner==current["old_owner"] and task.current_lease_version==current["lease_version"] and task.id in old.task_ids and (current["new_owner"] is None or new is not None and new.id==current["new_owner"] and new.state is not WorkerState.RETIRED):
    ...
    def attach_request_store(self, repo_root:Path|str) -> RequestAudit:
        ...
        if not isinstance(self.request_lifecycle_ledger,Ledger) or self.request_lifecycle_ledger.root!=resolved:
            raise InvariantError("request continuity requires a construction-bound Ledger for the same repo root")
        self.request_store=RequestStore(resolved); self.request_continuity_enabled=True; audit=self.request_audit(0); self.request_feed_sequence_floor=max((record.transitions[-1].cursor.feed_sequence for record in audit.records),default=0); return audit
    ...
    def _request_ledger(self):
        if self.request_lifecycle_ledger is None: raise InvariantError("canonical Ledger is not attached")
        return self.request_lifecycle_ledger
    @staticmethod
    ...
    def _validate_request_state(self, state:dict, projection:dict|None=None) -> tuple[RequestRecord,...]:
        if projection is None and self.request_lifecycle_ledger is None:
        ...
        records=tuple(self._raw_to_record(current[identity]["record"]) for identity in state["order"] if identity in current and current[identity]["record"] is not None)
        ...
        inbox=state["inbox"]
        if len({stage.request_id for stage in stages})!=len(stages) or any(set(raw)!={"stage_id","task_id","owner","contract_digest"} or raw["stage_id"] not in state["stages"] or state["stages"][raw["stage_id"]]["request_id"]!=identity for identity,raw in inbox.items()): raise InvariantError("request transport inbox does not match its stages")
        if len(accepted)!=sum(stage.state is RequestStageState.ACCEPTED for stage in stages) or set(accepted)!={record.id for record in records} or any(accepted[record.id].task_id!=record.task_id or accepted[record.id].owner!=record.accepted_owner or accepted[record.id].history[-1][1]!=record.transitions[0].cursor.event_receipt for record in records): raise InvariantError("accepted request stage does not match its Ledger record")
        events=projection["events"]
        for request_id,acknowledgements in state["acknowledgements"].items():
            for acknowledgement in acknowledgements:
                event=events.get(acknowledgement["event_id"])
                if event is None or event["request_id"]!=request_id or event["event_digest"]!=acknowledgement["event_digest"] or event["event_seq"]!=acknowledgement["event_seq"]: raise InvariantError("request acknowledgement does not bind a retained Ledger event")
        return records
    def _request_snapshot(self) -> tuple[dict,str,tuple[RequestRecord,...]]:
        state,digest=self._request_store().read(); projection=self._request_ledger().project_request_lifecycles()
        missing=any(event_id not in {item["event_id"] for item in state["acknowledgements"].get(event["request_id"],())} for event_id,event in projection["events"].items())
        pending_accept=any(item["record"] is not None and self._raw_to_stage(item["stage_id"],state["stages"][item["stage_id"]]).state is RequestStageState.PROVISIONAL for item in projection["records"])
        if missing or pending_accept:
            def recover(value):
                for event_id,event in sorted(projection["events"].items(),key=lambda item:int(item[1]["event_seq"])):
                    journal=value["acknowledgements"].setdefault(event["request_id"],[])
                    if event_id not in {item["event_id"] for item in journal}: journal.append({"event_id":event_id,"event_digest":event["event_digest"],"event_seq":event["event_seq"]})
                for item in projection["records"]:
                    if item["record"] is None: continue
                    stage=value["stages"][item["stage_id"]]
                    if stage["state"]=="PROVISIONAL": stage.update(state="ACCEPTED",history=[stage["history"][0],["ACCEPTED",item["record"]["transitions"][0]["cursor"]["event_receipt"]]])
    ...
    def _request_matches_live(self, state:dict, record:RequestRecord) -> bool:
        ...
        return any(value.startswith("usr-") for value in event.proof_receipts) and (record.state is RequestState.CANCELLED or record.successor_id in {item.id for item in self._validate_request_state(state)})
    ...
    def _append_request_lifecycle(self, state:dict, digest:str, request_id:str, lifecycle_state:str, identity:str, record:RequestRecord|None, *, accept_stage:bool=False) -> RequestView|None:
        projection=self._request_ledger().project_request_lifecycles(); current=next((item for item in projection["records"] if item["request_id"]==request_id),None)
        inbox=state["inbox"].get(request_id)
        if inbox is None: raise InvariantError("request lifecycle requires its retained transport envelope")
        event_id="request-event:"+sha256(f"{request_id}:{identity}".encode()).hexdigest(); dedupe_key="request-dedupe:"+sha256(f"{request_id}:{identity}".encode()).hexdigest()
        if event_id in projection["events"]:
            if current is None or current["event_id"]!=event_id or current["lifecycle_state"]!=lifecycle_state or current["record"]!=(None if record is None else self._record_to_raw(record)): raise InvariantError("request lifecycle replay conflicts with retained Ledger identity")
            return None if record is None else RequestView(state["sequence"],digest,record)
        payload={"schema_version":1,"record_type":"REQUEST_LIFECYCLE","event_id":event_id,"dedupe_key":dedupe_key,"request_id":request_id,"stage_id":inbox["stage_id"],"parent_event_id":None if current is None else current["event_id"],"envelope_digest":inbox["contract_digest"],"lifecycle_state":lifecycle_state,"record":None if record is None else self._record_to_raw(record),"route_receipt_ids":[],"permitted_route_ids":[],"failed_goal_turn_receipt_ids":[],"release_authority":None,"release_receipt_id":None,"release_issued_at_ms":None}
        try: appended=self._request_ledger().append_request_lifecycle(payload)
        except Exception as error: raise InvariantError(str(error)) from error
        cursor=appended["cursor"]
        def acknowledge(value):
            journal=value["acknowledgements"].setdefault(request_id,[])
            if event_id not in {item["event_id"] for item in journal}: journal.append({"event_id":event_id,"event_digest":cursor["event_digest"],"event_seq":cursor["event_seq"]})
            if accept_stage:
                stage=value["stages"][inbox["stage_id"]]
                if stage["state"]=="PROVISIONAL": stage.update(state="ACCEPTED",history=[stage["history"][0],["ACCEPTED",record.transitions[0].cursor.event_receipt]])
        try:
            final_state,final,_=self._request_store()._mutate_validated(acknowledge,expected=(state["sequence"],digest)); self._validate_request_state(final_state)
        except RequestStoreError as error: raise InvariantError(str(error)) from error
    ...
    def stage_request_task(self, actor:Role, task:Task) -> RequestStage:
        ...
        if existing_stage is None:
            ...
            def write(value): value["stages"][stage_id]={"task_id":task.id,"owner":owner,"contract_digest":contract,"state":"PROVISIONAL","request_id":request_id,"history":[["PROVISIONAL",contract]]}; value["inbox"][request_id]={"stage_id":stage_id,"task_id":task.id,"owner":owner,"contract_digest":contract}; value["acknowledgements"][request_id]=[]; value["order"].append(request_id)
            state,digest,_=self._mutate_request(write,expected=(state["sequence"],digest))
        else:
            stage_id,raw=existing_stage; request_id=raw["request_id"]; contract=self._request_contract_digest(task,request_id)
            if raw["owner"]!=owner or raw["contract_digest"]!=contract: raise InvariantError("provisional request transport conflicts with current task custody")
            stage=self._raw_to_stage(stage_id,raw)
        if not any(item["request_id"]==request_id for item in self._request_ledger().project_request_lifecycles()["records"]): self._append_request_lifecycle(state,digest,request_id,"OFFERED",contract,None)
    ...
    def _request_transition(self, actor:Role, request_id:str, prior:set[RequestState], event_receipt:str, kinds:set[CtrlFeedEventKind], *, next_state:RequestState|None=None, successor_id:str="", due:RequestDue|None=None, owner:bool=True, user:bool=False, fresh_proof:bool=True, append_evidence:bool=True):
        ...
        if retained is not None:
            if retained.cursor!=cursor: raise InvariantError("request transition replay conflicts with retained cursor")
            return state,digest,record,task,event,True
        if cursor.feed_sequence<=record.transitions[-1].cursor.feed_sequence: raise InvariantError("request transition requires a later unused published event")
    ...
    def resume_request(self, actor:Role, request_id:str, event_receipt:str, due:RequestDue) -> RequestView:
        ...
        record=record if replay else replace(record,next_due_event=due.event,next_due_at=due.at); return self._write_request(state,digest,record,"RUNNING",event.receipt)
    def supersede_request(self, actor:Role, request_id:str, successor_id:str, event_receipt:str) -> RequestView:
        self._role(actor,{Role.CTRL}); state,digest,record,_,event,_=self._request_transition(actor,request_id,{RequestState.OPEN,RequestState.BLOCKED},event_receipt,{CtrlFeedEventKind.DECISION},next_state=RequestState.SUPERSEDED,successor_id=successor_id,owner=False,user=True); return self._write_request(state,digest,record,"COMPLETE",event.receipt)
    ...
    def reprioritize_requests(self, actor:Role, unresolved_ids:tuple[str,...]) -> RequestAudit:
        ...
        def write(payload:dict):
            records={item.id:item for item in self._validate_request_state(payload)}; current=tuple(identity for identity in payload["order"] if identity in records and records[identity].state in {RequestState.OPEN,RequestState.BLOCKED})
    ...
    def record_host_custody_receipt(self, actor:Role, receipt:HostCustodyReceipt|None) -> HostCustodyReceipt:
        self._role(actor,{Role.CTRL})
        ledger=self.request_lifecycle_ledger
        if ledger is None or not hasattr(ledger,"retain_host_custody_receipt") or not isinstance(receipt,HostCustodyReceipt): raise InvariantError("HUMAN_AUTHORITY_BLOCKER: no host-pinned trust root or IPC verifier is configured; custody remains UNVERIFIED")
        return ledger.retain_host_custody_receipt(receipt)
    def require_host_custody(self, actor:Role, mutation:CustodyMutation, target_id:str, now:int, *, target_state_digest:str, receipt:HostCustodyReceipt|None=None) -> HostCustodyReceipt:
        ...
        ledger=self.request_lifecycle_ledger
        if ledger is None or not hasattr(ledger,"require_host_custody_receipt"): raise InvariantError("HUMAN_AUTHORITY_BLOCKER: no host-pinned trust root or IPC verifier is configured; archive custody remains UNVERIFIED")
        return ledger.require_host_custody_receipt(receipt,target_id=target_id,binding=target_state_digest,issued_at=now)
    ...
    def _record(self, target:str, item:object) -> None:
        ...
        limit=self.task_event_limit if target=="events" else 64
        if target=="events" and (not isinstance(limit,int) or isinstance(limit,bool) or not 8<=limit<=256): raise InvariantError("task event limit must be an integer from 8 to 256")
    ...
    def resolve_control_path_failure(self, actor:Role, failure:ControlPathFailure, *, same_owner_route:str="", safely_resumable:bool=False, authorized_handoff_owner:str="", successor_prohibited:bool=False, replacement_prohibited:bool=False, disjoint_ready_task_ids:tuple[str,...]=(), user_keep_out:bool|None=None, recovery_exhausted:bool|None=None, release_condition:str="", responsible_authority:str="", release_receipt:HostCustodyReceipt|None=None, release_receipt_id:str="", eta_range_ms:tuple[int,int]|None=None, eta_receipt_id:str="", material_receipt:MaterialStateReceipt|None=None, cost_receipt:AttemptCostReceipt|None=None) -> ControlPathRecoveryDecision:
        """Preserve delegated custody and choose one receipt route after an empty host completion."""
        self._role(actor,{Role.CTRL,Role.LEAD})
        if not isinstance(failure,ControlPathFailure) or failure.task_id not in self.tasks: raise InvariantError("control-path recovery requires an existing exact delegated task")
        task=self.tasks[failure.task_id]; self._validate_delegation_contract(task); contract=task.delegation_contract
        if contract is None: raise InvariantError("control-path recovery requires a delegated return contract")
        if task.state in {TaskState.COMPLETE,TaskState.ARCHIVED,TaskState.ARCHIVED_STALE}: raise InvariantError("terminal delegated work cannot enter control-path recovery")
        if failure.owner_id!=task.owner or failure.owner_id!=contract.owner_id or failure.dispatch_receipt_id!=task.subagent_receipt: raise InvariantError("control-path failure must bind the accepted dispatch and exact current owner")
        if failure.artifact!=contract.artifact or failure.artifact_paths!=contract.artifact_paths: raise InvariantError("control-path failure must preserve the exact candidate and custody paths")
        if user_keep_out is not None: raise InvariantError("control-path user keep-out is derived from durable task custody, not caller assertion")
        if recovery_exhausted is not None: raise InvariantError("control-path exhaustion is derived from retained route and goal-turn history, not caller assertion")
        handoff=authorized_handoff_owner.strip()
        if handoff and (not task.reviewer or handoff!=task.reviewer or handoff in {task.owner,task.creator}): raise InvariantError("control-path handoff requires the existing authorized exact independent owner")
        ...
        release_id=""
        if release_receipt is not None:
            ledger=self._task_handoff_ledger(); binding=self.retry_topology_ledger.control_path_release_binding(failure,same_owner_route=same_owner_route,safely_resumable=safely_resumable,authorized_handoff_owner=handoff,release_condition=release_condition,responsible_authority=responsible_authority)
            ledger.require_host_custody_receipt(release_receipt,target_id=failure.task_id,binding=binding,not_before=failure.observed_at_ms); release_id=release_receipt.receipt
        custody=self.retry_topology_ledger._issue_custody_proof(task.id,self._task_custody_digest(task),self._control_path_user_keep_out(task),release_id)
        decision=self.retry_topology_ledger.control_path_failure(failure,same_owner_route=same_owner_route,safely_resumable=safely_resumable,authorized_handoff_owner=handoff,successor_prohibited=successor_prohibited,replacement_prohibited=replacement_prohibited,disjoint_ready_task_ids=disjoint_ready_task_ids,custody_proof=custody,release_condition=release_condition,responsible_authority=responsible_authority,release_receipt=release_receipt,release_receipt_id=release_receipt_id,eta_range_ms=eta_range_ms,eta_receipt_id=eta_receipt_id,material_receipt=material_receipt,cost_receipt=cost_receipt)
    ...
    def _control_path_user_keep_out(self, task:Task) -> bool:
        return any((task.user_custody_required,task.user_state_changed,task.user_renamed,task.user_pinned,task.user_choice_pending))  # stale evidence cannot downgrade keep-out
    ...
    def choose_retry_reassessment(self, choice:ReassessmentChoice) -> ReassessmentChoice:
        receipt=choice.host_custody_receipt
        current=False
        if receipt is not None and self.request_lifecycle_ledger is not None and hasattr(self.request_lifecycle_ledger,"require_host_custody_receipt"):
            try:
                pending=self.retry_topology_ledger._pending
                if pending is not None: self.request_lifecycle_ledger.require_host_custody_receipt(receipt,target_id=choice.target,binding=self.retry_topology_ledger.host_binding_digest(pending,choice)); current=True
            except Exception:
                current=False
