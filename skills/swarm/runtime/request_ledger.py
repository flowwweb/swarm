...
def _canonical(value:dict)->bytes: return json.dumps(value,sort_keys=True,separators=(",",":"),ensure_ascii=True).encode()
class RequestStore:
    ...
    def decode(self,raw:bytes)->dict:
        try: value=json.loads(raw.decode()) if raw else {"version":1,"sequence":0,"order":[],"requests":{},"stages":{}}
    ...
    def read(self)->tuple[dict,str]:
        with self.state.locked():
            value=self.decode(self.state.read_bytes_unlocked()); return deepcopy(value),sha256(_canonical(value)).hexdigest()
    def peek(self)->tuple[dict,str,bool]:
        if not self.state.path.exists(): value={"version":1,"sequence":0,"order":[],"requests":{},"stages":{}}; return value,sha256(_canonical(value)).hexdigest(),False
        ...
        value=self.decode(raw); return deepcopy(value),sha256(_canonical(value)).hexdigest(),True
    def _mutate_validated(self,callback,expected:tuple[int,str]|None=None)->tuple[dict,str,object]:
        with self.state.locked():
            value=self.decode(self.state.read_bytes_unlocked()); before=_canonical(value); digest=sha256(before).hexdigest()
            if expected is not None and expected!=(value["sequence"],digest): raise RequestStoreError("stale request ledger identity")
            candidate=deepcopy(value); result=callback(candidate); candidate["sequence"]+=1; payload=_canonical(self.decode(_canonical(candidate))); self.state.replace_bytes_unlocked(payload); return deepcopy(candidate),sha256(payload).hexdigest(),result
    def with_current(self,expected:tuple[int,str]|None,callback):
        with self.state.locked():
            value=self.decode(self.state.read_bytes_unlocked()); digest=sha256(_canonical(value)).hexdigest()
            if expected is not None and expected!=(value["sequence"],digest): raise RequestStoreError("stale request ledger identity")
