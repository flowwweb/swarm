...
class InternalApprovalFallbackContractTests(unittest.TestCase):
    def test_internal_helper_approval_falls_back_without_waiting_on_user(self) -> None:
        ...
        self.assertRegex(skill, r"(?is)Internal-helper or read-only-tool approval gates are failed capacity.*cancel the attempt.*host gate.*without asking the user")
    ...
    def test_genuine_user_authority_approval_still_blocks(self) -> None:
        ...
        self.assertRegex(skill, r"(?is)never grants external, provider, destructive, or user-reserved authority")
