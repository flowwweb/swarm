...
Use the shallowest structure that can finish the accepted objective. `CTRL_DIRECT` is only one low-risk atomic outcome on one mutable surface, no cross-lane dependency, and measurable completion within the configured horizon; otherwise use `CTRL_DELEGATED` and a LEAD. By default, each delegated or non-CTRL task delegates one bounded outcome-critical slice to a subagent before substantive work. The owner integrates and accepts; CTRL and MOTHER do not quietly become the worker. Record only the typed exceptions in the hierarchy contract. Internal-helper or read-only-tool approval gates are failed capacity: cancel the attempt, record the host gate, and continue direct bounded owner work without asking the user. This never grants external, provider, destructive, or user-reserved authority.

Resolve new assignment model/reasoning from the active profile, route tier, and global execution bounds. Treat fast-tier and turbo settings as host-dependent preferences, never proof or a safety exception. Load [model-providers.md](references/model-providers.md) when selecting or probing a model/provider.
...
Run one event-driven goal clock, owned by CTRL, with a locally chosen horizon no longer than 60 minutes. At the due event inspect raw diff, process, tests, dependencies, and artifacts; do not early-poll or accept activity prose. One documented readiness delay permits one evidence-bound reschedule; then apply the miss ladder and keep goal history intact. Load [monitoring.md](references/monitoring.md) for watchdog, recovery, heartbeat, and feed-audit behavior.

## ROLE

Every role contract states only PURPOSE, OWNERSHIP, BOUNDARIES, and ESCALATION. CTRL owns intake, durable objective ledger, topology, and human route. MOTHER owns portfolio topology/integration/acceptance. LEAD owns one mutable lane, integration, gates, and lane completion. DOER owns one bounded artifact. A persistent SPECIALIST owns one named cross-cutting truth surface and gate; EXPERT is bounded advice without artifact ownership; SUPERVISOR recovers repeated misses without taking implementation, specialist, review, topology, or acceptance authority; independent REVIEW verifies. No prompt transfers authority between roles.
