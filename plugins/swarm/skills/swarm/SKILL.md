...
SWARM is a Codex-native multi-agent workflow and plugin. Codex is the only
agent host whose task, tool, model, and execution behavior SWARM describes or
routes. The `plugins/swarm` directory is the generated Codex marketplace mirror
of this canonical skill. External-provider language below is reserved for real
service proof (such as auth, payment, deployment, or browser evidence); it is
...
Before the first mutable handoff, select the most efficient and smallest graph that can complete the objective durably, reliably, and confidently. Follow [graph-engineering.md](references/graph-engineering.md): one CTRL root, explicit artifact-owning lanes, parallelize only independent work, serialize shared-surface gates, and keep every edge explainable by a dependency or acceptance reason. Before creating visible lanes, compile the runtime `TopologyMaterializationPlan`: exactly one CTRL remains the project administrator; every LEAD and DOER has a typed profession, parent, bounded responsibility, and generated profession-plus-structural-role title; a LEAD has typed durable-boundary evidence or one declared direct artifact; and CTRL fanout beyond `preferred_lane_width` requires a concrete span exception receipt. Convert only the current ready wave into one `TopologyDispatchPacket` and reserve every lane identity before a host create attempt. A child waits for a later wave until its parent has a retained host-confirmed identity; a confirmed lane is never prepared again. Review additionally requires a fresh runtime-issued `TopologyArtifactFreezeReceipt` bound to the exact producer lane, content-observed artifact, proof plan, independent review, and topology plan—an artifact ID or caller-created label never qualifies. Ambiguous or failed transport keeps the reservation pending; do not retry another schema until it resolves or is explicitly cancelled. The current Codex `create_thread` surface cannot consume this packet, so it remains instruction-only and live topology enforcement is `UNVERIFIED`. Never create a flat crowd of bare LEADs. A game objective selects the registered `game_studio` graph: game-studio lead, design, engineering, art, audio, playtest/QA, and release agents with production dependencies; medium and large lanes are visible Codex tasks, while lane-local bounded work may use subagents. Do not flatten a domain graph into a single undifferentiated worker list.
...
An explicit request for a mockup, UI mockup, screen option, or design option
deterministically requests built-in ImageGen bitmap output. Designer generates
one separate full-frame bitmap for every requested frame or option; CTRL only
routes the request and surfaces the resulting artifacts. A composite, collage,
contact sheet, stitched screenshot, HTML/CSS/canvas rendering, or other
code-native substitute is permitted only when the user explicitly asks for
that artifact class or explicitly approves an exception. Even then, a
supplemental composite never replaces any separately requested frame or option.
...
Apply the [user expectation loop](references/review-contract.md#user-expectation-loop)
at intake, delegation, recovery, and acceptance: identify the user and purpose,
define observable success, and compare the final integrated result with that
expectation. Carry this through the existing task contract for every role.

Every SWARM agent and profession follows [runtime-recovery.md](references/runtime-recovery.md)
when a failure, disputed blocker, uncertain permission, timeout, or repeated
non-progress needs reassessment. Rebind current authority, separate facts from
assumptions, test the disputed claim, and take one evidence-backed next action;
use the existing owner/reviewer escalation if the confusion survives.
...
Review and correction must be materially worthwhile. Outside design, require a concrete consequence and observable improvement that outweighs the work. Block only material user impact, correctness, safety, regression, maintainability, reference fidelity, or an explicit acceptance failure; cosmetic preference is an optional note and never another loop unless the user requested that polish. Choose the smallest durable solution: reuse, delete, or consolidate before adding abstractions, roles, process, tests, or dependencies. Classify each instruction or failure in its named scope, fix it locally, preserve accepted product character and useful capability, and regression-check the adjacent behavior at risk instead of inferring an opposite blanket rule. Design may refine granular craft within the accepted direction, never unrelated scope; use [design-guide.md](references/design-guide.md). Repeated findings follow the shared [recovery loop](references/runtime-recovery.md); preserve the configured model and reasoning unless the user authorizes a change. Correct doctrine at its lowest existing governing layer and consolidate rather than add incident clauses. Confidence never permits fabricated proof, weakened security, or custody bypass. Load the review contract for the full significance, visual-proof, escaped-defect, and independent-review procedure.
...
Render decisive proof inside the CTRL feed response itself. Use conversation-native image, audio, generated-image, or visualization content so the user can inspect it at a glance without opening a path; tool previews, local-path Markdown, file links, and commentary that disappears behind the final response do not count as surfaced proof. Keep links only as provenance or fallback. Surface every requested mockup, screen, frame, or design option as its own full-frame inline artifact with a concise label and claim limit. A composite or contact sheet may supplement those separate artifacts only when explicitly requested; it never substitutes for them. If the host cannot render the content inline, report that exact delivery blocker and provide the best clickable fallback, but leave the result unsurfaced and acceptance open.

When material previews form one decision set, surface each promptly with a concise label and known defect. Do not create a consolidated decision gallery unless the user explicitly requests that artifact class; when requested, it remains supplemental to the separately surfaced candidates. A representative subset is allowed only for a genuinely large non-option evidence set the user did not ask to see in full, with the complete inventory and exact omissions. Links or an inventory alone cannot accept a decision set. At the explicit selection boundary, record exactly one `SELECTED` candidate and mark every other inventoried candidate `REJECTED`; rejected local binaries become Storage cleanup candidates only after the selected artifact is durably preserved and hash-bound. Use [decision-set.md](references/decision-set.md) for the no-glob, no-worker, no-process lifecycle. Load [review-contract.md](references/review-contract.md) for visual evidence and [monitoring.md](references/monitoring.md) for feed receipts and correction behavior.
